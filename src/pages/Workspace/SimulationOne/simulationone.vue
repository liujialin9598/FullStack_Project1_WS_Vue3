<template>
  <div class="page" v-loading="loading">
    ...
    <br /><br />
    <p>
      The following graphs illustrate the distribution of lifetime wellbeing and
    ...
    <div class="bars">
      <div class="basicbar">
        <BasicBar
          ...
          xname="Lifetime Wellbeing"
          title="Lifetime Wellbeing distribution"
          right_grid="25%"
        ></BasicBar>
      </div>
      <div class="basicbar">
    ...
    <!-- #region Lifetime Wellbeing -->
    <h1>Lifetime Wellbeing</h1>
    <p>
    ...
    <div class="charts2">
      <div class="function">
        ...
        <div>
          <Formular
            :formula="[
              ['W<sub>L</sub>', 'Lifetime wellbeing'],
    ...
    <!-- #region wellbeing  -->
    <h1>Wellbeing functions</h1>
    <br />
    <p><b>Wellbeing each year</b></p>
    <div class="charts2">
      <div class="function">
        <div style="text-align: left">
          <img
    ...
    <div class="charts2">
      <div class="function">
        <div style="text-align: left">
          <img
            style="height: 60px"
            src="@/assets/formulars/return.png"
            alt="Price"
          />
        </div>
        <br />
        <div>
          <Formular
            :formula="[
              ...
              ['RIR', 'The real interest rate, which is constant across time'],
              ['MRP', 'Market Risk Premium'],
              [
                'σ<sub>port</sub>',
                'Standard deviation of annual return on members balance',
              ],
              ['W<sub>t</sub>', 'a draw from the standard normal distribution'],
            ]"
            :left_length="0.3"
          ></Formular>
        </div>
      ...
      <div class="parameters">
        ...
        <br />
        <p>σport</p>
        <el-input v-model="apiData.σport.value" />
        <br />
        <p>RIR</p>
        <el-input v-model="apiData.RIR.value" />
        <br />
        <p>MRP</p>
        <el-input v-model="apiData.MRP.value" />
        <br />
...
onBeforeMount(async () => {
  try {
    ...
    if (!localapiDataStr || localapiDataStr.length < 5) {
      const response = await axios.get("/api/sone/");
      //存储本地数据
    ...
    if (!localonesimData || localonesimData.length < 5) {
      const response = await axios.get("/api/sone/onesim/", {
        params: apiData,
      });
    ...
    if (!localmultisimData || localmultisimData.length < 5) {
      const response = await axios.get("/api/sone/multisim/", {
        params: apiData,
      });
